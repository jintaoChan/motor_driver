...
__weak void R3_2_GetPhaseCurrents( PWMC_Handle_t * pHdl, ab_t * pStator_Currents )
{
  ...
  hReg1 = ( pHandle->pParams_str->ADCDataReg1[bSector]->JDR1 ) << 1;
  hReg2 = ( pHandle->pParams_str->ADCDataReg2[bSector]->JDR1 ) << 1;
...
__weak void R3_2_GetPhaseCurrents_OVM( PWMC_Handle_t * pHdl, ab_t * Iab )
{
  ...
  /* ADC value needs to be left shifted once to remove sign extension - Specific to F4XX and F7XX */
  ADCDataReg1 = ( pHandle->pParams_str->ADCDataReg1[Sector]->JDR1 ) << 1;
  ADCDataReg2 = ( pHandle->pParams_str->ADCDataReg2[Sector]->JDR1 ) << 1;
...
__weak void R3_2_HFCurrentsCalibrationAB( PWMC_Handle_t * pHdl, ab_t * pStator_Currents )
{
  ...
  {
    pHandle->PhaseAOffset += pHandle->pParams_str->ADCDataReg1[pHandle->PolarizationSector]->JDR1;
    pHandle->PhaseBOffset += pHandle->pParams_str->ADCDataReg2[pHandle->PolarizationSector]->JDR1;
    pHandle->PolarizationCounter++;
  }
...
__weak void R3_2_HFCurrentsCalibrationC( PWMC_Handle_t * pHdl, ab_t * pStator_Currents )
{
  ...
  if ( pHandle->PolarizationCounter < NB_CONVERSIONS )
  {
    pHandle->PhaseCOffset += pHandle->pParams_str->ADCDataReg2[pHandle->PolarizationSector]->JDR1;
    pHandle->PolarizationCounter++;
  }
 ...
 * 
 * @param pHdl: Handler of the current instance of the PWM component.
 * @retval Return value of R3_2_WriteTIMRegisters.
 */
...
void R3_2_RLGetPhaseCurrents( PWMC_Handle_t * pHdl, ab_t * pStator_Currents )
{
  ...
  /* ADC value needs to be left shifted once to remove sign extension - Specific to F4XX and F7XX */
   wAux = (int32_t)( pHandle->PhaseBOffset ) - (int32_t)( ( pHandle->pParams_str->ADCDataReg2[pHandle->_Super.Sector]->JDR1) << 1 );
