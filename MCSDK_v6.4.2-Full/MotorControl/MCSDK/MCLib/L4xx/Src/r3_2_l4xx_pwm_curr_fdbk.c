...
__weak void R3_2_Init( PWMC_R3_2_Handle_t * pHandle )
{
  ...
  if ( ( uint32_t )pHandle == ( uint32_t )&pHandle->_Super )
  {
    ...
    while( LL_ADC_INJ_IsStopConversionOngoing( ADCx_1 ) & LL_ADC_INJ_IsStopConversionOngoing( ADCx_2 ) )
    {
      /* Nothing to do */
    }
  
    /* Set trigger mode to hardware to avoid instant conversion */
    LL_ADC_INJ_SetTriggerEdge( ADCx_1, LL_ADC_INJ_TRIG_EXT_RISING );
    LL_ADC_INJ_SetTriggerEdge( ADCx_2, LL_ADC_INJ_TRIG_EXT_RISING );

    /* Start injected conversion */
    LL_ADC_INJ_StartConversion( ADCx_1 );
    LL_ADC_INJ_StartConversion( ADCx_2 );
    
    /* Generate event to flush the JSQR without removing the JADSTART bit */
