...
{
  return (SPD_GetAvrgMecSpeedUnit(${SPD_AUX_SUPER_M1}));
}
...
{
  int16_t returnValue;
  returnValue = (SPD_GetAvrgMecSpeedUnit(${SPD_AUX_SUPER_M1}) * U_RPM) / SPEED_UNIT;
...
{
  return (SPD_GetElAngle(${SPD_AUX_SUPER_M1}));
}
...
{
  return (((float_t)SPD_GetElAngle(${SPD_AUX_SUPER_M1})) / S16ToRAD);
}
</#if>
...
{
  return (SPD_GetAvrgMecSpeedUnit(${SPD_AUX_SUPER_M2}));
}

/**
 * @brief Returns the last computed average mechanical rotor speed from auxiliary sensor for Motor 2, expressed in RPM
 */
__weak float_t MC_GetMecAuxiliarySpeedAverageMotor2_F(void)
{
  return ((float_t)(SPD_GetAvrgMecSpeedUnit(${SPD_AUX_SUPER_M2}) * U_RPM) / SPEED_UNIT);
}
...
{
  return (SPD_GetElAngle(${SPD_AUX_SUPER_M2}));
}

/**
 * @brief Returns the electrical angle of the rotor from auxiliary sensor of Motor 2, expressed in radians
 */
__weak float_t MC_GetAuxiliaryElAngleMotor2_F(void)
{
  return ((float_t)((SPD_GetElAngle(${SPD_AUX_SUPER_M2})) / S16ToRAD));
}
</#if>
...
<#if MC.M1_DRIVE_TYPE == "FOC" && AUX_SPEED_FDBK_M1 && !MC.M1_POSITION_CTRL_ENABLING >
/**
...
/**
  ...
  * starting up the motor. 
  *
  * @note The MCI_StartMotor2 command only triggers the execution of the start-up 
  * procedure (or eventually the offset calibration procedure) and returns 
  * immediately after. It is not blocking the execution of the application until 
  * the motor is indeed running in steady state. If the application needs to wait 
  * for the motor to be running in steady state, the application has to check the 
...
/**
  ...
  * command was executed or discarded.
  *  
  * @note The MC_StopMotor2() command only triggers the stop motor procedure 
  * and then returns. It is not blocking the application until the motor is indeed 
  * stopped. To know if it has stopped, the application can query the motor's state 
  * machine and check if the #IDLE state has been reached.
  *
...
/**
  ...
  * with the provided parameters. The programmed ramp is executed immediately if
  * Motor 2's state machine is in the #RUN states. Otherwise, the ramp is buffered 
  * and will be executed when the state machine reaches any of the aforementioned state.
  *
...
/**
  ...
  *         value.
  */
__weak void MC_ProgramSpeedRampMotor2_F(float_t FinalSpeed, uint16_t hDurationms)
{
...
/**
  ...
  *         value.
  */
__weak void MC_ProgramTorqueRampMotor2_F(float_t FinalTorque, uint16_t hDurationms)
{
...
/**
  ...
  *         It is possible to set 0 to perform an instantaneous angular change (follow mode).
  */
__weak void MC_ProgramPositionCommandMotor2(float_t fTargetPosition, float_t fDuration)
{
...
/**
  ...
  * with the provided parameters. The programmed reference is executed immediately if
  * Motor 2's state machine is in the #RUN states. Otherwise, the command is buffered
  * and will be executed when the state machine reaches any of the aforementioned state.
  *
  *  The Application can check the status of the command with the MC_GetCommandStateMotor2()
  * to know whether the last command was executed immediately or not.
 ...
 */
__weak float_t MC_GetMecSpeedReferenceMotor2_F(void)
{
 ...
 */
__weak float_t MC_GetAverageMecSpeedMotor2_F(void)
{
 ...
 * @brief Returns the final speed of the last ramp programmed for Motor 2 if this ramp was a speed ramp, 0 otherwise.
 */
__weak float_t MC_GetLastRampFinalSpeedMotor2_F(void)
{
  return (MCI_GetLastRampFinalSpeed_F(pMCI[M2]));
}

/**
 * @brief Returns the final torque reference for Motor 2, expressed in Ampere.
 */
__weak float_t MC_GetFinalTorqueReferenceMotor2_F(void)
{
  return (MCI_GetLastRampFinalTorque_F(pMCI[M2]));
}

/**
 * @brief Returns the final torque reference for Motor 2, expressed in digit.
 */
__weak int16_t MC_GetFinalTorqueReferenceMotor2(void)
{
  return (MCI_GetLastRampFinalTorque(pMCI[M2]));
}
...
/**
 * @brief returns Iq and Id reference current values for Motor 2 in float_t type
 */
 ...
 * @brief returns the electrical torque reference for Motor 2 
 */
__weak float_t MC_GetTerefMotor2_F(void)
{
  return (MCI_GetTeref_F(pMCI[M2]));
}
/**
 ...
 * @retval float_t The average measured motor power expressed in watt.
 */
__weak float_t MC_GetAveragePowerMotor2_F(void)
{
 ...
 *   */
__weak float_t MC_GetCurrentPosition2(void)
{
 ...
 *   */
__weak float_t MC_GetTargetPosition2(void)
{
 ...
 *   */
__weak float_t MC_GetMoveDuration2(void)
{
  return (MCI_GetMoveDuration(pMCI[M2]));
}

/**
 * @brief returns the current estimated angle of the Motor 2.
 *   */
__weak float_t MC_GetCtrlPositionAngle2(void)
{
  return (MCI_GetCtrlPositionAngle(pMCI[M2]));
}

/**
 * @brief sets the current estimated anglular position of the Motor 2.
 * @param  fCurrentPosition current mechanical angular position (in radian).
 *   */
__weak void MC_SetCtrlPositionAngle2(float_t fCurrentPosition)
{
  MCI_SetCtrlPositionAngle(pMCI[M2], fCurrentPosition);
}

  </#if><#-- MC.M2_POSITION_CTRL_ENABLING == true -->
  <#if MC.M2_DRIVE_TYPE == "FOC" && AUX_SPEED_FDBK_M2 && !MC.M2_POSITION_CTRL_ENABLING >
/**
 * @brief This method is used to switch between the primary and the auxiliary sensor
 *        for Motor 2.
 * @param  deltaAngleMargin maximum delta between primary and auxiliary sensor to allow switching.
 * @retval returns true if the command is successfully executed, false otherwise.
 */
__weak bool MC_SensorSwitchMotor2(uint16_t deltaAngleMargin)
{
   return (MCI_SensorSwitch(pMCI[M2], deltaAngleMargin));
}

/**
 * @brief This method is used to set the primary sensor used by FOC algo for Motor 2.
 * @param  deltaAngleMargin maximum delta between primary and auxiliary sensor to allow switching.
 * @param  newSensor new primary sensor used by FOC algo 
 * @retval returns true if the command is successfully executed, false otherwise.
 *
 * example is provided in [Auxiliary speed sensor usage](rotor_aux_speed_pos_feedback.md)
 */
__weak bool MC_SensorSetMotor2(uint16_t deltaAngleMargin, SpeednPosFdbk_Handle_t * newSensor)
{
   return (MCI_SensorSet(pMCI[M2], deltaAngleMargin, newSensor));
}
  </#if>  
</#if> <#-- MC.DRIVE_NUMBER > 1> -->
/**
