...
<#if CondBootHookUsed>
  <#if MC.M1_POTENTIOMETER_ENABLE == true>
    <#if MC.M1_SPEED_SENSOR == "HSO" || MC.M1_SPEED_SENSOR == "ZEST">
    PotentiometerSetScaleFactor(&potentiometer_M1, &flashParams);
...
<#if CondPostMediumFrequencyHookM2Used>
  <#if MC.M2_POTENTIOMETER_ENABLE == true>
  uint16_t rawValue = RCM_GetRegularConv(&PotRegConv_M2);
  (void)SPDPOT_Run(&SpeedPotentiometer_M2, rawValue);
  </#if><#-- MC.M2_POTENTIOMETER_ENABLE == true -->
