...
<#if CondFamily_STM32H5 || CondFamily_STM32U5>
...
static void R1_SetAOReferenceVoltage(DAC_TypeDef *DACx, uint32_t DAC_Channel, uint16_t hDACVref);
  </#if><#--  MC.M1_OCP_TOPOLOGY == "EMBEDDED" || MC.M2_OCP_TOPOLOGY == "EMBEDDED"
           || MC.INTERNAL_OVERVOLTAGEPROTECTION == true || MC.INTERNAL_OVERVOLTAGEPROTECTION2 == true> -->
</#if><#-- CondFamily_STM32F3 || CondFamily_STM32G4 || CondFamily_STM32H5 -->
...
<#if MC.M1_OCP_COMP_SRC == "DAC" || MC.M2_OCP_COMP_SRC == "DAC">
    DAC_TypeDef *DACx = pHandle->pParams_str->DACx;
</#if><#-- MC.M1_OCP_COMP_SRC=="DAC" || MC.M2_OCP_COMP_SRC=="DAC" -->     
<#if MC.M1_OCP_TOPOLOGY == "EMBEDDED" || MC.M2_OCP_TOPOLOGY == "EMBEDDED" ||
    MC.INTERNAL_OVERVOLTAGEPROTECTION == true || MC.INTERNAL_OVERVOLTAGEPROTECTION2 == true>
...
<#if MC.M1_OCP_TOPOLOGY == "EMBEDDED" || MC.M2_OCP_TOPOLOGY == "EMBEDDED">
  ...
  /********************************************************************************************/
    ...
    if (COMP_OCPx)
    {
      ...
      {
         R1_SetAOReferenceVoltage(DACx, (uint32_t)(pHandle->pParams_str->DACChannelX), (uint16_t)(pHandle->pParams_str->DAC_OCP_Threshold));
      }
      else
      {
        /* Nothing to do */
...
<#if MC.INTERNAL_OVERVOLTAGEPROTECTION == true || MC.INTERNAL_OVERVOLTAGEPROTECTION2 == true>
  ...
  /*                                      ADC Initialization                                  */
  /********************************************************************************************/
<#if CondFamily_STM32F3 || CondFamily_STM32L4 || CondFamily_STM32G0 || CondFamily_STM32G4 || CondFamily_STM32F7 || CondFamily_STM32H5 || CondFamily_STM32C0 || CondFamily_STM32U5>
...
<#if CondFamily_STM32G4 || CondFamily_STM32H5 || CondFamily_STM32U5>
...
<#if (CondFamily_STM32F7 == false) && (CondFamily_STM32H5 == false) && (CondFamily_STM32U5 == false)>   
...
<#if CondFamily_STM32F3 || CondFamily_STM32L4 || CondFamily_STM32G4 || CondFamily_STM32H5 || CondFamily_STM32U5>       
  /* Start injected conversion */
  LL_ADC_INJ_StartConversion(ADCx);
<#elseif CondFamily_STM32F7>
#if (defined (STM32F756xx) || defined (STM32F746xx) || defined (STM32F745xx))
  /* Workaround software to fix hardware bug on STM32F7xx
   * set trigger detection to rising and falling edges to be able to
   * trigger ADC on TRGO2
   * refer to errata sheet ES0290 */
  LL_ADC_INJ_StartConversionExtTrig(ADCx, LL_ADC_INJ_TRIG_EXT_RISINGFALLING);
...
void R1_1ShuntMotorVarsInit(PWMC_Handle_t *pHdl)
{
  ...
  if (pHandle->_Super.ChargeBootCap == false)
  {
    ...
    pHandle->_Super.CntPhA = pHandle->_Super.ChargeBootCapDutyCycles;
    pHandle->_Super.CntPhB = pHandle->_Super.ChargeBootCapDutyCycles;
    pHandle->_Super.CntPhC = pHandle->_Super.ChargeBootCapDutyCycles;
  }
...
/**
  ...
  *            @arg DAC_Channel_x: DAC Channelx selected.
  * @param  DACx: DAC to be configured.
  * @param  hDACVref: Value of DAC reference expressed as 16bit unsigned integer. \n
  *         Ex. 0 = 0V ; 65536 = VDD_DAC.
  */
void R1_SetAOReferenceVoltage(DAC_TypeDef *DACx, uint32_t DAC_Channel, uint16_t hDACVref)
{
...
<#if CondFamily_STM32F7>
      ...
      LL_DMA_SetDataLength(DMAx, pHandle->pParams_str->DMAChannelX, DMA_TRANSFER_LENGTH);
      LL_DMA_EnableStream(DMAx, pHandle->pParams_str->DMAChannelX);
<#elseif CondFamily_STM32H5 || CondFamily_STM32U5><#-- CondFamily_STM32H5 -->
      /* Set DMA settings */
      LL_DMA_DisableChannel(DMAx, pHandle->pParams_str->DMAChannelX);
      LL_DMA_SetSrcAddress(DMAx, pHandle->pParams_str->DMAChannelX, (uint32_t)&(pHandle->DmaBuffCCR[0]));
      LL_DMA_SetBlkDataLength(DMAx, pHandle->pParams_str->DMAChannelX, DMA_TRANSFER_LENGTH );
