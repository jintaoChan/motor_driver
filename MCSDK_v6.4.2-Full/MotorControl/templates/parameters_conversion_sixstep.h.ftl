...
#define PWM_PERIOD_CYCLES_REF                (uint16_t)(((uint32_t)ADV_TIM_CLK_CM_MHz * (uint32_t)1000000u\
...
/*************** Timer for IPD PWM generation  ******/
#define IPD_PWM_PERIOD_CYCLES                (uint16_t)(((uint32_t)ADV_TIM_CLK_MHz * (uint32_t)1000000u\
                                             / ((uint32_t)(M1_IPD_PWM_FREQ_HZ))) & (uint16_t)0xFFFE)
#define IPD_PWM_DUTY_CYCLE                   (uint16_t)((IPD_PWM_PERIOD_CYCLES * M1_IPD_PWM_DUTY_CYCLE_PERC / 100U) + 1U)
#define IPD_ADC_SAMPLING_TSAMPLING_TIM_TICKS (uint16_t)(((M1_IPD_ADC_BEMF_NUM_TS_CYCLES + 0.5) * ADV_TIM_CLK_MHz) / ADC_CLK_MHz)
</#if><#-- MC.M1_IPD_STARTUP == true-->
