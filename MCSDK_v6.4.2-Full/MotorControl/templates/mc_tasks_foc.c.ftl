...
<#if M1_IS_STO_CORDIC || M1_IS_STO_PLL || M2_IS_STO_CORDIC || M2_IS_STO_PLL>
...
<#if MC.M2_COMPLEX_GATE_DRIVER_INTERFACE != "NONE">
    ${MC.M2_PWM_DRIVER_PN?upper_case}_StopManagement(&${MC.M2_PWM_DRIVER_PN?upper_case}_Handle_M2);
</#if>  
  }
  <#else>
  ${M1_CS_TOPO}SwitchOffPWM(pwmcHandle[motor]);
...
__weak void TSK_MediumFrequencyTaskM1(void)
{
  ...
  int16_t wAux = 0;
<#if (MC.M1_PWM_DRIVER_PN == "STDRIVE101" || MC.M1_PWM_DRIVER_PN == "STSPIN32G4") && (MC.M1_OTF_STARTUP == true)>
  PWMC_${M1_CS_TOPO}Handle_t *p_Handle = (PWMC_${M1_CS_TOPO}Handle_t *)pwmcHandle[M1];
</#if><#-- (MC.M1_PWM_DRIVER_PN == "STDRIVE101" || MC.M1_PWM_DRIVER_PN == "STSPIN32G4") && (MC.M1_DP_TOPOLOGY != "NONE") && (MC.M1_OTF_STARTUP == true) -->
<#if MC.M1_DBG_OPEN_LOOP_ENABLE == true>
...
<#if MC.M1_ICL_ENABLED == true>
        ...
        case IDLE:
        {
          ...
          {
<#if AUX_SPEED_FDBK_M1 == true>
            pSTC[M1]->AUXSPD = (SpeednPosFdbk_Handle_t *) &${GetSpeedSensorInstance('M1',true)};
</#if>
<#if MC.M1_COMPLEX_GATE_DRIVER_INTERFACE != "NONE">
...
<#if (MC.M1_SPEED_SENSOR == "STO_PLL") || (MC.M1_SPEED_SENSOR == "STO_CORDIC") >
  ...
  <#if MC.M1_DBG_OPEN_LOOP_ENABLE == true>
            ...
            {
<#if MC.M1_PWM_DRIVER_PN == "STSPIN32G4" || MC.M1_PWM_DRIVER_PN == "STDRIVE101">
...
<#if MC.M1_DP_DESTINATION == "TIM_BKIN" && MC.M1_PWM_DRIVER_PN == "STDRIVE101">
                LL_TIM_DisableBRK(${_last_word(MC.M1_PWM_TIMER_SELECTION)});
<#elseif MC.M1_DP_DESTINATION == "TIM_BKIN2" && MC.M1_PWM_DRIVER_PN == "STDRIVE101">
...
<#else><#-- MC.M1_PWM_DRIVER_PN != "STSPIN32G4" || MC.M1_PWM_DRIVER_PN != "STDRIVE101" -->              
              (void)PWMC_CurrentReadingCalibr(pwmcHandle[M1], CRC_START);
              Mci[M1].State = OFFSET_CALIB;
</#if><#-- MC.M1_PWM_DRIVER_PN == "STSPIN32G4" || MC.M1_PWM_DRIVER_PN == "STDRIVE101" -->
            }
...
<#if MC.M1_SPEED_FEEDBACK_CHECK == true || (MC.M1_SPEED_SENSOR == "HALL_SENSOR")>
    ...
    <#else>
              if(!SPD_Check((SpeednPosFdbk_Handle_t *)${SPD_M1}))
    </#if>
              {
...
<#if MC.MOTOR_PROFILER == true || MC.ONE_TOUCH_TUNING == true || M1_ENCODER>
  ...
  <#elseif ((MC.M1_AUXILIARY_SPEED_SENSOR == "QUAD_ENCODER") || (MC.M1_AUXILIARY_SPEED_SENSOR == "QUAD_ENCODER_Z")) && (MC.M1_SPEED_SENSOR != "HALL_SENSOR")>
            if (TSK_StopPermanencyTimeHasElapsedM1())
            {
              RUC_Clear(&RevUpControlM1, MCI_GetImposedMotorDirection(&Mci[M1]));
              ${SPD_clear_M1}(${SPD_M1});
...
<#if MC.M2_ICL_ENABLED == true>
        ...
        case IDLE:
        {
          ...
          {
<#if AUX_SPEED_FDBK_M2 == true>
            pSTC[M2]->AUXSPD = (SpeednPosFdbk_Handle_t *) &${GetSpeedSensorInstance('M2',true)};
</#if>		  
<#if MC.M2_COMPLEX_GATE_DRIVER_INTERFACE != "NONE">
            ${MC.M2_PWM_DRIVER_PN?upper_case}_StartManagement(&${MC.M2_PWM_DRIVER_PN?upper_case}_Handle_M2);
</#if>  
<#if (MC.M2_SPEED_SENSOR == "STO_PLL") || (MC.M2_SPEED_SENSOR == "STO_CORDIC")>
  <#if MC.M2_DBG_OPEN_LOOP_ENABLE == true>
            if ( mode != MCM_OPEN_LOOP_VOLTAGE_MODE && mode != MCM_OPEN_LOOP_CURRENT_MODE) 
...
<#else><#-- MC.MOTOR_PROFILER == false -->
  ...
  <#if (MC.M2_SPEED_SENSOR == "QUAD_ENCODER") || (MC.M2_SPEED_SENSOR == "QUAD_ENCODER_Z")>
    <#if MC.M2_DBG_OPEN_LOOP_ENABLE>
              if (mode == MCM_OPEN_LOOP_VOLTAGE_MODE || mode == MCM_OPEN_LOOP_CURRENT_MODE)
              { 
                MCI_ExecBufferedCommands(&Mci[M2]); /* Exec the speed ramp even in OL*/
                Mci[M2].State = RUN;              
              }
              else
              {
  ...
  <#elseif  (MC.M2_AUXILIARY_SPEED_SENSOR == "QUAD_ENCODER") || (MC.M2_AUXILIARY_SPEED_SENSOR == "QUAD_ENCODER_Z")>
    ...
    <#if MC.M2_SPEED_SENSOR == "HALL_SENSOR">  
                STC_SetControlMode(pSTC[M2], MCM_SPEED_MODE);
                STC_SetSpeedSensor(pSTC[M2], &HALL_M2._Super);
                FOC_InitAdditionalMethods(M2);
                FOC_CalcCurrRef(M2);
                STC_ForceSpeedReferenceToCurrentSpeed( pSTC[M2]); /* Init the reference speed to current speed */
                MCI_ExecBufferedCommands(&Mci[M2]); /* Exec the speed ramp after changing of the speed sensor */
  ...
  <#if MC.M2_OTF_STARTUP == false && MC.MOTOR_PROFILER == false>
        ...
        case SWITCH_OVER:
        {
          ...
          else
          {
            ...
            /* Compute the virtual speed and positions of the rotor.
               The function returns true if the virtual speed is in the reliability range */
            bool FlagEnableClosedLoop = VSS_CalcAvrgMecSpeedUnit(&VirtualSpeedSensorM2, &hForcedMecSpeedUnit);
            /* Check if the transition ramp has completed. */
            bool FlagTransitionPhaseCompleted = VSS_TransitionEnded(&VirtualSpeedSensorM2);
            FlagEnableClosedLoop = FlagEnableClosedLoop || FlagTransitionPhaseCompleted;
...
<#if MC.M2_SPEED_FEEDBACK_CHECK == true || (MC.M2_SPEED_SENSOR == "HALL_SENSOR")>
    <#if AUX_SPEED_FDBK_M2 == true>
              if(!SPD_Check(STC_GetSpeedSensor(pSTC[M2])))
    <#else>
              if(!SPD_Check((SpeednPosFdbk_Handle_t *) ${SPD_M2}))
    </#if>              
              {
...
<#if MC.MOTOR_PROFILER == true || MC.ONE_TOUCH_TUNING == true || M2_ENCODER>
  ...
  <#elseif ((MC.M2_AUXILIARY_SPEED_SENSOR == "QUAD_ENCODER") || (MC.M2_AUXILIARY_SPEED_SENSOR == "QUAD_ENCODER_Z")) && (MC.M2_SPEED_SENSOR != "HALL_SENSOR")>
            if (TSK_StopPermanencyTimeHasElapsedM2())
            {
              RUC_Clear(&RevUpControlM2, MCI_GetImposedMotorDirection(&Mci[M2]));
              ${SPD_clear_M2}(${SPD_M2});
...
<#else><#-- MC.DRIVE_NUMBER > 1 -->
  ...
  else
  {
    ...
    else /* bMotorNbr != M1 */
    {
      ...
      <#if (MC.M2_SPEED_SENSOR == "STO_PLL") || (MC.M2_SPEED_SENSOR == "STO_CORDIC")>
      if (true == AngleSpeedEstimation[Mci[M2].State])
      {
      ...
      <#if AUX_SPEED_FDBK_M2 == true>
      if (RUN == Mci[M2].State) {
        FOCVars[M2].AuxDeltaAngle=SPD_GetElAngle(STC_GetSpeedSensor(pSTC[M2]))-SPD_GetElAngle(STC_GetAuxSensor(pSTC[M2]));
      }
      </#if> <#--AUX_SPEED_FDBK_M2 == true -->	  
    }
  }
...
inline uint16_t FOC_CurrControllerM1(void)
{
  ...
  else
  {
    ...
    LL_ADC_SetSamplingTimeCommonChannels (ADC1, PWM_Handle_M1.pParams_str->ISamplingTime);
       <#elseif CondFamily_STM32G0 || CondFamily_STM32C0>
    /* restore phase current sampling channel */    
    LL_ADC_REG_SetSequencerChannels(ADC1, __LL_ADC_DECIMAL_NB_TO_CHANNEL (PWM_Handle_M1.pParams_str->IChannel));
       </#if><#-- CondFamily -->
    </#if> <#-- single shunt -->
...
<#if MC.M2_DBG_OPEN_LOOP_ENABLE == true>
  ...
  Ialphabeta = MCM_Clarke(Iab);
  Iqd = MCM_Park(Ialphabeta, hElAngle);
  if (PWMC_GetPWMState(pwmcHandle[M2]) == true)
  {
  ...
  if (PWMC_GetPWMState(pwmcHandle[M2]) == true)
  {
    hCodeError = PWMC_SetPhaseVoltage${OVM}(pwmcHandle[M2], Valphabeta);
  }
  else
  {
    /* Nothing to do. No PWM setting to prevent possible ChargeBootCap conflict */ 
