...
<#if FOC && AUX_SPEED_FDBK_M1 && !MC.M1_POSITION_CTRL_ENABLING > 
...
float_t MC_GetAveragePowerMotor2_F(void);
  ...
  <#if MC.M2_POSITION_CTRL_ENABLING == true>
/* returns the current control position state of Motor 2 */
...
<#if FOC && AUX_SPEED_FDBK_M2 && !MC.M2_POSITION_CTRL_ENABLING > 
/* Switch the primary sensor used by FOC with the auxiliary sensor for motor 2 */
bool MC_SensorSwitchMotor2(uint16_t deltaAngleMargin);

/* Set the primary sensor used by FOC alo for motor 2 */
bool MC_SensorSetMotor2(uint16_t deltaAngleMargin, SpeednPosFdbk_Handle_t * newSensor);
</#if>


/* returns the current angle of position control algorithm of Motor 2. */
float_t MC_GetCtrlPositionAngle2(void);

/* sets the current angle of position control algorithm of Motor 2. */
void MC_SetCtrlPositionAngle2(float_t fCurrentPosition);
