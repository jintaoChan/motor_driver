<#ftl strip_whitespace = true>
<#include "*/ftl/header.ftl">
<#include "*/ftl/common_assign.ftl">
<#include "*/ftl/common_fct.ftl">
/**
  ******************************************************************************
  * @file    pwm_common.c
  * @author  Motor Control SDK Team, ST Microelectronics
  * @brief   This file provides firmware functions that implement common features
  *          of the PWM & Current Feedback component of the Motor Control SDK:
  *
  *           * start timers (main and auxiliary) synchronously
  *
  ******************************************************************************
  * @attention
  *
  * <h2><center>&copy; Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.</center></h2>
  *
  * This software component is licensed by ST under Ultimate Liberty license
  * SLA0044, the "License"; You may not use this file except in compliance with
  * the License. You may obtain a copy of the License at:
  *                             www.st.com/SLA0044
  *
  ******************************************************************************
  * @ingroup pwm_curr_fdbk
  */

/* Includes ------------------------------------------------------------------*/
#include "pwm_common.h"

/** @addtogroup MCSDK
  * @{
  */

/** @addtogroup ConvFOC
  * @{
  */

/** @addtogroup MCLLAPI
  * @{
  */

/** @addtogroup pwm_curr_fdbk
  * @{
  */

<#if NoInjectedChannel>
/**
  * @brief  Performs the start of all the timers required by the control.
  * 
  * Uses TIM2 as a temporary timer to achieve synchronization between PWM signals.
  * When this function is called, TIM1 and/or TIM8 must be in a frozen state
  * with CNT, ARR, REP RATE and trigger correctly set (these settings are
  * usually performed in the Init method accordingly with the configuration)
  */
__weak void startTimers(void)
{
  LL_TIM_EnableCounter(${_last_word(MC.M1_PWM_TIMER_SELECTION)});
}
<#else>  
#ifdef TIM2
/**
  * @brief  Performs the start of all the timers required by the control.
  * 
  * Uses TIM2 as a temporary timer to achieve synchronization between PWM signals.
  * When this function is called, TIM1 and/or TIM8 must be in a frozen state
  * with CNT, ARR, REP RATE and trigger correctly set (these settings are
  * usually performed in the Init method accordingly with the configuration)
  */
__weak void startTimers(void)
{
  uint32_t isTIM2ClockOn;
  uint32_t trigOut;

  <#if !(MC.M1_SPEED_SENSOR== "HSO" || MC.M1_SPEED_SENSOR== "ZEST")>
/* Motor 1 ADC interrupt */
  LL_ADC_ClearFlag_JEOS(${MC.ADC_PERIPH});
  LL_ADC_EnableIT_JEOS(${MC.ADC_PERIPH});
    <#if (MC.DRIVE_NUMBER != "1") && (MC.ADC_PERIPH != MC.ADC_PERIPH2)>

  /* Motor 2 ADC interrupt */
  LL_ADC_ClearFlag_JEOS(${MC.ADC_PERIPH2});
  LL_ADC_EnableIT_JEOS(${MC.ADC_PERIPH2});

    </#if>
  </#if>
  /* Motor 1 timer slave mode ITR1 (TIMER2) sensitive */ 
  LL_TIM_SetTriggerInput(${_last_word(MC.M1_PWM_TIMER_SELECTION)}, LL_TIM_TS_ITR1);
  LL_TIM_SetSlaveMode(${_last_word(MC.M1_PWM_TIMER_SELECTION)}, LL_TIM_SLAVEMODE_TRIGGER);

  <#if MC.M1_SPEED_SENSOR== "HSO" || MC.M1_SPEED_SENSOR== "ZEST">
  /* Motor 1 auxiliary timer slave mode ITR1 (TIMER2) sensitive */
  LL_TIM_SetTriggerInput(TIM3, LL_TIM_TS_ITR1);
  LL_TIM_SetSlaveMode(TIM3, LL_TIM_SLAVEMODE_TRIGGER);
  </#if>

  <#if CondFamily_STM32F3 || CondFamily_STM32F4 || CondFamily_STM32G4>
    <#if MC.DRIVE_NUMBER != "1">
  /* Motor 2 timer slave mode ITR1 (TIMER2) sensitive */ 
  LL_TIM_SetTriggerInput(${_last_word(MC.M2_PWM_TIMER_SELECTION)}, LL_TIM_TS_ITR1);
  LL_TIM_SetSlaveMode(${_last_word(MC.M2_PWM_TIMER_SELECTION)}, LL_TIM_SLAVEMODE_TRIGGER);
  
    </#if><#-- MC.DRIVE_NUMBER != "1" -->
  </#if><#-- CondFamily_STM32F3 || CondFamily_STM32F4 || CondFamily_STM32G4 -->
  isTIM2ClockOn = LL_APB1_GRP1_IsEnabledClock(LL_APB1_GRP1_PERIPH_TIM2);
  if ((uint32_t)0 == isTIM2ClockOn)
  {
    /* Temporary Enable TIM2 clock if not already on */
    LL_APB1_GRP1_EnableClock(LL_APB1_GRP1_PERIPH_TIM2);
    LL_TIM_GenerateEvent_UPDATE(TIM2);
    LL_APB1_GRP1_DisableClock(LL_APB1_GRP1_PERIPH_TIM2);
  }
  else
  {
    trigOut = LL_TIM_ReadReg(TIM2, CR2) & TIM_CR2_MMS;
    LL_TIM_SetTriggerOutput(TIM2, LL_TIM_TRGO_UPDATE);
    LL_TIM_GenerateEvent_UPDATE(TIM2);
    LL_TIM_SetTriggerOutput(TIM2, trigOut);
  }

}
#endif
</#if><#-- NoInjectedChannel -->

/**
  * @brief  Waits for the end of the polarization. 
  * 
  * If the polarization exceeds the number of needed PWM cycles, it reports an error.
  * 
  * @param  TIMx Timer used to generate PWM.
  * @param  SWerror Variable used to report a SW error.
  * @param  repCnt Repetition counter value.
  * @param  cnt Polarization counter value.
  */
//cstat !MISRAC2012-Rule-8.13
__weak void waitForPolarizationEnd(TIM_TypeDef *TIMx, uint16_t  *SWerror, uint8_t repCnt, volatile uint8_t *cnt)
{
#ifdef NULL_PTR_CHECK_POW_COM
  if ((MC_NULL == cnt) || (MC_NULL == SWerror))
  {
    /* Nothing to do */
  }
  else
  {
#endif
    uint16_t hCalibrationPeriodCounter;
    uint16_t hMaxPeriodsNumber;

    hMaxPeriodsNumber = ((uint16_t)2 * NB_CONVERSIONS) * (((uint16_t)repCnt + 1U) >> 1);

    /* Wait for NB_CONVERSIONS to be executed */
    LL_TIM_ClearFlag_CC1(TIMx);
    hCalibrationPeriodCounter = 0u;
    while (*cnt < NB_CONVERSIONS)
    {
      if ((uint32_t)ERROR == LL_TIM_IsActiveFlag_CC1(TIMx))
      {
        LL_TIM_ClearFlag_CC1(TIMx);
        hCalibrationPeriodCounter++;
        if (hCalibrationPeriodCounter >= hMaxPeriodsNumber)
        {
          if (*cnt < NB_CONVERSIONS)
          {
            *SWerror = 1u;
            break;
          }
          else
          {
            /* Nothing to do */
          }
        }
        else
        {
          /* Nothing to do */
        }
      }
      else
      {
        /* Nothing to do */
      }
    }
#ifdef NULL_PTR_CHECK_POW_COM
  }
#endif
  }

/**
  * @}
  */

/**
  * @}
  */

/************************ (C) COPYRIGHT 2026 STMicroelectronics *****END OF FILE****/
