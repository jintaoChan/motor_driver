...
<#if CondFamily_STM32U5><#-- CondFamily_STM32U5 --->
  <#if ((MC.M1_CURRENT_SENSING_TOPO == 'SINGLE_SHUNT_PHASE_SHIFT') || (MC.M1_CURRENT_SENSING_TOPO == 'SINGLE_SHUNT_ACTIVE_WIN'))>
#include "r1_ps_pwm_curr_fdbk.h"
  </#if><#-- ((MC.M1_CURRENT_SENSING_TOPO == 'SINGLE_SHUNT_PHASE_SHIFT') || (MC.M1_CURRENT_SENSING_TOPO == 'SINGLE_SHUNT_ACTIVE_WIN')) -->
  <#if MC.M1_CURRENT_SENSING_TOPO == 'ICS_SENSORS'>
#error ics U5 not defined
  </#if><#-- MC.M1_CURRENT_SENSING_TOPO == 'ICS_SENSORS' -->
  <#if (MC.M1_CURRENT_SENSING_TOPO=='THREE_SHUNT') && (MC.M1_CS_ADC_NUM=='1')>
#include "r3_1_u5xx_pwm_curr_fdbk.h"
  </#if><#-- (MC.M1_CURRENT_SENSING_TOPO=='THREE_SHUNT') && (MC.M1_CS_ADC_NUM=='1') -->
...
/* PWM generation parameters --------------------------------------------------*/
  ...
  .TIMx              = ${_last_word(MC.M2_PWM_TIMER_SELECTION)},
  .Tcase2            = ((uint16_t)TDEAD2 + (uint16_t)TNOISE2 + (uint16_t)TW_BEFORE_R3_1_2) / 2u,
  .Tcase3            = (uint16_t)TW_BEFORE_R3_1_2 + (uint16_t)TDEAD2 + (uint16_t)TRISE2,
...
<#if CondFamily_STM32C0>
  <#if ((MC.M1_CURRENT_SENSING_TOPO == 'THREE_SHUNT') && ( MC.M1_CS_ADC_NUM == '1' ))>
/**
  * @brief  Current sensor parameters Single Drive - three shunt, STM32C0X
  */
const R3_1_Params_t R3_1_Params =
{
/* PWM generation parameters --------------------------------------------------*/
...
<#if CondFamily_STM32U5><#-- CondFamily_STM32U5 --->
  <#if  MC.M1_CURRENT_SENSING_TOPO=='THREE_SHUNT'><#-- Inside CondFamily_STM32U5 --->
    <#if MC.M1_CS_ADC_NUM =='1'><#-- Inside CondFamily_STM32U5 --->
/**
  * @brief  Current sensor parameters Motor 1 - three shunt 1 ADC STM32U5 Family
  */
const R3_1_Params_t R3_1_ParamsM1 =
{
/* Current reading A/D Conversions initialization -----------------------------*/
...
/* PWM generation parameters --------------------------------------------------*/
  ...
  .TIMx              = ${_last_word(MC.M1_PWM_TIMER_SELECTION)},
    ...
    </#if><#-- MC.M1_CS_ADC_NUM =='1' --><#-- CondFamily_STM32U5 --->
  </#if><#-- MC.M1_CURRENT_SENSING_TOPO == 'ICS_SENSORS' --><#-- CondFamily_STM32U5 --->
</#if><#-- CondFamily_STM32U5 --->
...
/* PWM generation parameters --------------------------------------------------*/
  ...
  .Tafter                = TW_AFTER2,
  .Tbefore               = TW_BEFORE2,
  .Tcase2                = ((uint16_t)TDEAD2 + (uint16_t)TNOISE2 + (uint16_t)TW_BEFORE2) / 2u,
  .Tcase3                = (uint16_t)TW_BEFORE2+ (uint16_t)TDEAD2 + (uint16_t)TRISE2,
  .TIMx                  = ${_last_word(MC.M2_PWM_TIMER_SELECTION)},
...
/* Internal COMP settings ----------------------------------------------------*/   
  ...
  <#if MC.M1_OCP_COMP_SRC == "DAC">
/* DAC settings --------------------------------------------------------------*/
  .DACx                  = ${MC.M1_OCP_DAC_U},
  .DACChannelX           = LL_DAC_CHANNEL_${_last_word(MC.M1_OCP_DAC_CHANNEL_U, "OUT")},
  .DAC_OCP_Threshold     = ${MC.M1_DAC_CURRENT_THRESHOLD},
...
/* Internal COMP settings ----------------------------------------------------*/
  ...
  <#if MC.M2_OCP_COMP_SRC=="DAC">
/* DAC settings --------------------------------------------------------------*/
  .DACx                  = ${MC.M2_OCP_DAC_U},
  .DACChannelX           = LL_DAC_CHANNEL_${_last_word(MC.M2_OCP_DAC_CHANNEL_U, "OUT")},
  .DAC_OCP_Threshold     = ${MC.M2_DAC_CURRENT_THRESHOLD},
  </#if><#-- MC.M2_OCP_COMP_SRC=="DAC" -->
