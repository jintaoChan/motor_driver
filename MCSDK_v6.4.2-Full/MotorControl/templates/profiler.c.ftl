  ...
  case PROFILER_STATE_FluxEstim:
    if ((handle->fluxestimObj.state == PROFILER_FLUXESTIM_STATE_Complete) || (handle->fluxestimObj.state == PROFILER_FLUXESTIM_STATE_Error))
    {
      ...
      if (handle->fluxestimObj.state == PROFILER_FLUXESTIM_STATE_Complete)
      {
        ...
        handle->maxSpeedHz = (FIXP30_toF(motor->pVBus->Udcbus_in_pu) * handle->fullScaleVoltage_V) / (1.732f * handle->debug_Flux_VpHz);
<#if MC.M1_POTENTIOMETER_ENABLE == true>        
        if (handle->maxSpeedHz > handle->fullScaleFreq_Hz) 
        {
          motor->pPotentiometer->speedref_scalefactor = FIXP30(1.0f);
        }
        else
        {
          motor->pPotentiometer->speedref_scalefactor = FIXP30(handle->maxSpeedHz / (float_t)handle->fullScaleFreq_Hz);
        }        
</#if>
        ...
        // suggested Kp_mech = 5 * rated current / rated speed: rated current is about
          ...
          FIXPSCALED_floatToFIXPscaled(oneOverRatedFlux_pu, &motor->pSPD->oneOverFlux_pu_fps);
            ...
            motor->pSPD->flagDynamicZestConfig = false;
...
float_t PROFILER_getAcMeasurementTime(PROFILER_Handle handle)
{
  return PROFILER_DCAC_getAcMeasurementTime(&handle->dcacObj);
}

float_t PROFILER_getDcMeasurementTime(PROFILER_Handle handle)
{
	return PROFILER_DCAC_getDcMeasurementTime(&handle->dcacObj);
}
...
/**
  * @brief  Sets measurement time allowed to AC step
  * @param  pHandle Handle on the Profiler component
  * @param  time_seconds time in seconds for AC step
  */
void PROFILER_setAcMeasurementTime(PROFILER_Handle handle, const float_t time_seconds)
{
	PROFILER_DCAC_setAcMeasurementTime(&handle->dcacObj, time_seconds);
}

/**
  * @brief  Sets measurement time allowed to DC step
  * @param  pHandle Handle on the Profiler component
  * @param  time_seconds time in seconds for DC step
  */
void PROFILER_setDcMeasurementTime(PROFILER_Handle handle, const float_t time_seconds)
{
	PROFILER_DCAC_setDcMeasurementTime(&handle->dcacObj, time_seconds);
}
