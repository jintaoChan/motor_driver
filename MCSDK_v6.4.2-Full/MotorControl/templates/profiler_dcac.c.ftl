...
<#if MC.M1_SPEED_SENSOR == "HSO">
  PROFILER_IMPEDEST_init(&handle->impedestObj);
</#if>
}
...
void PROFILER_DCAC_setParams(PROFILER_DCAC_Handle handle, PROFILER_Params* pParams)
{
  ...
  PROFILER_DCAC_setDcMeasurementTime(handle, pParams->dcac_dc_measurement_time_s);
  PROFILER_DCAC_setAcMeasurementTime(handle, pParams->dcac_ac_measurement_time_s);
...
<#if MC.M1_SPEED_SENSOR == "HSO">
  PROFILER_IMPEDEST_setParams(&handle->impedestObj, pParams);
</#if>
}
...
void PROFILER_DCAC_run(PROFILER_DCAC_Handle handle, MOTOR_Handle motor)
{
  ...
  case PROFILER_DCAC_STATE_AC_Measuring:
    {
<#if MC.M1_SPEED_SENSOR == "HSO">
      fixp30_t inject = PROFILER_IMPEDEST_getInject(&handle->impedestObj);
      motor->pCurrCtrl->Ddq_ref_pu.D = handle->dc_duty_pu + inject;
...
<#if MC.M1_SPEED_SENSOR == "HSO">
  /* Call impedance estimator */
  Voltages_Uab_t Uab_emf_filt_pu = IMPEDCORR_getEmf_ab_filt(motor->pSPD->pImpedCorr);
  Currents_Iab_t Iab_filt_pu = IMPEDCORR_getIab_filt(motor->pSPD->pImpedCorr);
  PROFILER_IMPEDEST_run(&handle->impedestObj, Uab_emf_filt_pu.A, Iab_filt_pu.A);
</#if>
}
...
<#if MC.M1_SPEED_SENSOR == "HSO">
  PROFILER_IMPEDEST_setFlagInject(&handle->impedestObj, true);
<#else>
...
<#else>
  ...
  case PROFILER_DCAC_STATE_RampUp:
    {
      ...
      if ((Imag > handle->Id_dc_ref_pu) || (Pmag > handle->PowerDC_goal_pu) || (handle->dc_duty_pu == handle->duty_maxgoal_pu))
      {
        ...
        /* Set timer counter */
        handle->counter = handle->dc_measurement_counts;
  ...
  case PROFILER_DCAC_STATE_DC_Measuring:
    ...
    if (handle->counter == 0)
    {
      ...
      handle->counter = handle->ac_measurement_counts;

<#if MC.M1_SPEED_SENSOR == "HSO">
      PROFILER_IMPEDEST_setInjectFreq_kHz(&handle->impedestObj, handle->ac_freqkHz_pu);
      PROFILER_IMPEDEST_setInjectRef(&handle->impedestObj, handle->ac_duty_pu);
<#else>
  ...
  case PROFILER_DCAC_STATE_AC_Measuring:
    {
      ...
      {
<#if MC.M1_SPEED_SENSOR == "HSO">
        PROFILER_IMPEDEST_calculate(&handle->impedestObj);
        handle->Rs_inject = PROFILER_IMPEDEST_getRs(&handle->impedestObj);
        handle->Ls_inject = PROFILER_IMPEDEST_getLs(&handle->impedestObj);
<#else>
        ...
        /* Stop injection */
<#if MC.M1_SPEED_SENSOR == "HSO">
        PROFILER_IMPEDEST_setFlagInject(&handle->impedestObj, false);
        PROFILER_IMPEDEST_setInjectRef(&handle->impedestObj, 0);
<#else>
...
{
  return handle->dc_measurement_time_sec;
}
...
{
  return handle->ac_measurement_time_sec;
}
...
<#if MC.M1_SPEED_SENSOR == "ZEST">
  ...
  motor->pSPD->pRsTemp_params->RsRatedOhm = handle->Rs_dc;
  RSTEMP_setParams(motor->pSPD->pRsTemp, motor->pSPD->pRsTemp_params);
  ...
  motor->pFocVars->Rs_Rated_Ohm = FIXP20(handle->Rs_dc);
}

void PROFILER_DCAC_setDcMeasurementTime(PROFILER_DCAC_Handle handle, const float_t time_seconds)
{
  handle->dc_measurement_time_sec = time_seconds;
  handle->dc_measurement_counts = (uint32_t) (time_seconds * handle->background_rate_hz);
}
...
{
  handle->ac_measurement_time_sec = time_seconds;
  handle->ac_measurement_counts = (uint32_t) (time_seconds * handle->background_rate_hz);
}
