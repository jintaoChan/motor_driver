...
<#-- dual drive shared ADCs -->
      <#if MC.ADC_PERIPH != MC.ADC_PERIPH2>
      <#-- this case can happen in dual three shunt 2ADC/1ADC.Ex: 
           motor 1 uses ADCx and ADCy, and motor 2 uses ADCx or ADCy 
           WB can return ADC_PERIPH = ADCx and ADC_PERIPH2 = ADCy -->
  if (LL_ADC_IsActiveFlag_JEOS(${MC.ADC_PERIPH}))
  {  
    LL_ADC_ClearFlag_JEOS(${MC.ADC_PERIPH});
  }
  if (LL_ADC_IsActiveFlag_JEOS(${MC.ADC_PERIPH2}))
  {  
  ...
  (void)TSK_HighFrequencyTask();
    <#else>
<#-- dual drive independent ADCs -->
     <#if (MC.ADC_PERIPH == "ADC1") ||  (MC.ADC_PERIPH == "ADC2")>
  if (LL_ADC_IsActiveFlag_JEOS(${MC.ADC_PERIPH}))
  {
    LL_ADC_ClearFlag_JEOS(${MC.ADC_PERIPH});
    (void)TSK_HighFrequencyTask(M1);
  }
     </#if>
...
void TIMx_UP_M1_IRQHandler(void)
{
 ...
 /* USER CODE END  TIMx_UP_M1_IRQn 0 */
  ...
  (void)${M1_CS_TOPO}TIMx_UP_IRQHandler(&PWM_Handle_M1);
<#if MC.DRIVE_NUMBER != "1">
...
void TIMx_UP_M2_IRQHandler(void)
{
 ...
 /* USER CODE END  TIMx_UP_M2_IRQn 0 */
  ...
  (void)${M2_CS_TOPO}TIMx_UP_IRQHandler(&PWM_Handle_M2);
