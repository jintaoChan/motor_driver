...
<#if (MC.DBG_MCU_LOAD_MEASURE == true) || (MC.M1_SPEED_SENSOR == "ZEST")>      
...
<#if MC.M1_SPEED_SENSOR == "ZEST">  
          ...
          case MC_REG_HSO_ZEST_REVISION:
            *regdataU32 = ZEST_getRevision( (ZEST_Handle) NULL );
          break;
<#if McuName?ends_with("TxZ")>

          case MC_REG_ZEST_CPN_LIB_VERSION:
          {
            uint32_t ZEST_getLibVersion(void);
            *regdataU32 = ZEST_getLibVersion();
          }
          break;
</#if><#-- ZeST CPN -->
</#if><#-- MC.M1_SPEED_SENSOR == "ZEST" -->
