...
uint8_t RI_SetRegCommandParser (MCP_Handle_t * pHandle, uint16_t txSyncFreeSpace)
{
  ...
  else
  {
    ...
    uint8_t typeID;
    uint8_t motorID;
    uint8_t (*SetRegFcts[NBR_OF_MOTORS+1])(uint16_t, uint8_t, uint8_t*, uint16_t*, int16_t) = {&RI_SetRegisterGlobal<#list 1..(MC.DRIVE_NUMBER?number) as NUM>, &RI_SetRegisterMotor${NUM}</#list>};
...
uint8_t RI_GetRegCommandParser (MCP_Handle_t * pHandle, uint16_t txSyncFreeSpace)
{
  ...
  else
  {
    ...
    uint8_t typeID;
    uint8_t motorID;
    uint8_t (*GetRegFcts[NBR_OF_MOTORS+1])(uint16_t, uint8_t, uint8_t*, uint16_t*, int16_t) = {&RI_GetRegisterGlobal<#list 1..(MC.DRIVE_NUMBER?number) as NUM>, &RI_GetRegisterMotor${NUM}</#list>};
    pHandle->txLength = 0;
    while (rxLength > 0U)
    {
      dataElementID = (uint16_t *) rxData;
...
<#if FOC && ( AUX_SPEED_FDBK_M1 || AUX_SPEED_FDBK_M2) >	
