...
typedef enum
{
    FOC_RsEst,             /*!< @brief Old Rs estimation */
    FOC_RsTemp,            /*!< @brief New Rs temperature estimation */
    numFOC_RsEstimAlgo_Select
} FOC_RsEstimAlgo_Select_e;
</#if>
...
{
<#if MC.M1_SPEED_SENSOR == "ZEST">   
  FOC_ZESTControl_s zestControl;           /*!< @brief ZeST control structure. 
                                                       Only applicable when ZeST is activated */
  FOC_ZestFeedback_s zestFeedback;         /*!< @brief ZeST feedback structure. 
                                                       Only applicable when ZeST is activated */
  ZEST_Obj      *pZeST;                    /*!< @brief pointer on ZeST component.
                                                       Only applicable when ZeST is activated */
  RSEST_Obj     *pRsEst;                   /*!< @brief pointer on RsEst component.
                                                       Only applicable when ZeST is activated */
</#if>
...
<#if MC.M1_SPEED_SENSOR == "ZEST">  
  fixp30_t RsEstModel_HysteresisBand_pu;   /*!< @brief Hysteresis band used to switch between the Rs estimation models */
  fixp30_t RsEstModel_ThreshUp;            /*!< @brief Upper frequency threshold for switching to the RsTemp estimation model */
  fixp30_t RsEstModel_ThreshDn;            /*!< @brief Lower frequency threshold for switching to the Rs estimation model when Zest is active */
</#if>  
...
<#if MC.M1_SPEED_SENSOR == "ZEST">  
  FOC_RsEstimAlgo_Select_e RsEstModel;     /*!< @brief Selects the appropriate RS estimation model */
</#if>  
