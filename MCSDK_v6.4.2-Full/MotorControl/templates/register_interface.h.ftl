...
uint8_t RI_GetRegisterMotor${NUM}(uint16_t regID, uint8_t typeID, uint8_t * data, uint16_t *size, int16_t freeSpace);
  ...
  <#if MC.LOW_MEMORY_FOOTPRINT_REG == false >
...
uint8_t HF_GetPtrReg(uint16_t dataID, void **dataPtr);
  <#if (MC.DRIVE_NUMBER)?number gt 1 >
    <#list 1..(MC.DRIVE_NUMBER?number) as NUM >
uint8_t HF_GetPtrRegMotor${NUM}(uint16_t dataID, void **dataPtr);
    </#list>
  </#if>
