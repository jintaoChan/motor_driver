...
<#else>
#error PFC supported only for F3
</#if> 
}
...
#ifndef FULL_MISRA_C_COMPLIANCY_PFC
    wCurrentReference = wCurrentReference >> 15;
#else
...
#ifdef NOT_IMPLEMENTED /* For debug.  */
...
__weak void PFC_Task( PFC_Handle_t * pHandle )
{
  ...
  if ( pHandle->pVBS != MC_NULL )
  {
    ...
    pHandle->hMotorPower = (int16_t)PQD_GetAvrgElMotorPowerW( pHandle->pMPW1 );

    if ( pHandle->pMPW2 != MC_NULL)
    {
      pHandle->hMotorPower += (int16_t)PQD_GetAvrgElMotorPowerW( pHandle->pMPW2 );
    }
  }
  else
  {
