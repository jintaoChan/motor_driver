...
/**
  * @brief  todo
  */
void PotentiometerSetScaleFactor(Potentiometer_Handle_t *pHandle,  FLASH_Params_t const *flashParams )
{
  pHandle->speedref_scalefactor = FIXP30(flashParams->throttle.speedMaxRPM  / 60.0f * flashParams->motor.polePairs / flashParams->scale.frequency);
}
/**
  * @brief  todo
  */
void PotentiometerGetPosition(Potentiometer_Handle_t *pHandle )
{
  ...
  <#if (MC.POTENTIOMETER_ADC !=  MC.M1_CS_ADC_U) && (MC.POTENTIOMETER_ADC !=  MC.M1_CS_ADC_V) && (MC.POTENTIOMETER_ADC !=  MC.M1_CS_ADC_W)>
  pHandle->adcVal = RCM_GetRegularConv(&PotRegConv_M1);
  ...
  <#if MC.M1_SPEED_SENSOR == "HSO">  
    pHandle->position = FIXP_sat(pos, FIXP30(1.0f), FIXP30(0.05f)) * pHandle->direction; 
  <#elseif MC.M1_SPEED_SENSOR == "ZEST">
